...
class ChatClient:
    ...
    def stream(self, message, session_id):
        ...
        data = {"human_input": message, "session_id": session_id}
        special_char = None

        try:
            with requests.post(f"{self.server_url}/stream", headers=headers, data=json.dumps(data), timeout=self.timeout, stream=True) as chunked_response:
